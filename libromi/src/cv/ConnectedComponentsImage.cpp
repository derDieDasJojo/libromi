...
namespace romi {
        ...
        ConnectedComponentsImage::ConnectedComponentsImage(Image& image)
                : image_(), cc_(30), label_data_(image.data().size())
        {
                // Long hand until we sort out copy constructors in Image.
                image_ = image;
                std::vector<unsigned int> out_data(image_.width() * image_.height());
                cc_.connected(image_.export_byte_data().data(), out_data.data(),
                              (int)image_.width(), (int)image_.height(),
                              std::equal_to<unsigned char>(),
                              constant<bool,true>());
                std::copy(out_data.data(), out_data.data() + out_data.size(),
                          label_data_.data());
    }
