...
namespace romi::arm {
        ...
        {
                set_format();
                if (!set_jpeg_quality(85))
                        throw std::runtime_error("ImageEncoder::ImageEncoder: set_jpeg_quality");
        ...
        bool ImageEncoder::set_jpeg_quality(uint32_t value)
        {
                MMAL_STATUS_T status;                
                status = mmal_port_parameter_set_uint32(output_,
                                                        MMAL_PARAMETER_JPEG_Q_FACTOR,
                                                        value);
                return status_is_ok("set_jpeg_quality", status);
        }
