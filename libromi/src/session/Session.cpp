...
namespace romi {
        ...
        Session::Session(const rcom::ILinux& linux, const std::string& base_directory,
                ...
                : linux_(linux),
                  ...
                  meta_folder_(),
                  observation_id_(),
                  roverIdentity_(std::make_unique<RoverIdentityProvider>(device_data,
                                                                         softwareVersion)),
                  location_(location)
        {
        ...
        void Session::start(const std::string &observation_id)
        {
                ...
                std::string session_dir = (device_data_.RomiDeviceType()
                                           + separator
                                           + device_data_.RomiDeviceHardwareId()
                                           + separator
                                           + datatime);
                session_directory_.clear();
                session_directory_ = base_directory_ / session_dir;
                meta_folder_ = std::make_shared<MetaFolder>(roverIdentity_, location_,
                                                            session_directory_);
        }
